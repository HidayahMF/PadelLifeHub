      ...
      <main>
        ...
        <section class="relative overflow-hidden border-b-2 border-ink bg-surface">
          ...
          <div class="relative mx-auto max-w-6xl px-4 py-14 text-center lg:px-6 lg:py-20">
            ...
            <h1 class="mt-5 font-display text-4xl text-ink sm:text-5xl">
              {{ t('Why LifeHub exists.') }}
            </h1>
            <p class="mx-auto mt-4 max-w-2xl text-base font-medium text-ink-soft">
              {{ t('A personal life management platform that brings productivity, personal finance, habits, and goals into one place — with an AI assistant that understands it all.') }}
            </p>
          </div>
        </section>
        ...
        <section class="mx-auto max-w-6xl px-4 py-14 lg:px-6 lg:py-16">
          <div class="grid grid-cols-1 gap-12 lg:grid-cols-2">
            <div>
              <h2 class="font-display text-2xl text-ink sm:text-3xl">{{ t('What is LifeHub?') }}</h2>
              <div class="mt-4 space-y-4 text-sm font-medium leading-relaxed text-ink-soft">
                <p>
                  {{ t('LifeHub is an all-in-one personal dashboard for running your life. Instead of juggling a task app, a finance tracker, a habit tracker, and a notes app, LifeHub keeps everything in one place — and connects the dots between them.') }}
                </p>
                <p>
                  {{ t('It started with a simple problem: life data lives in too many apps. Your tasks say one thing, your bank account says another, and nothing talks to each other. LifeHub brings them together so you can see the whole picture instead of isolated fragments.') }}
                </p>
                <p>
                  {{ t('Today it includes personal finance, tasks, habits, goals, calendar, notes, wishlist & needs, Pomodoro, statistics, weekly and monthly reviews, and LifeHub AI — an assistant that reads your own data to answer questions and suggest next steps.') }}
                </p>
              </div>
            </div>
            ...
            <div class="space-y-4">
              <div class="rounded-card border-2 border-ink bg-surface p-5 shadow-soft">
                ...
                <p class="mt-2 text-sm font-medium leading-relaxed text-ink-soft">
                  {{ t('Personal productivity and personal finance are usually tracked in separate tools with separate logins and separate mental models — so “I have no idea where my money went” and “I keep missing my own deadlines” stay true no matter how many apps you install.') }}
                </p>
              </div>
              <div class="rounded-card border-2 border-ink bg-surface p-5 shadow-soft">
                <h3 class="flex items-center gap-2 font-display text-base text-ink">
                ...
                <p class="mt-2 text-sm font-medium leading-relaxed text-ink-soft">
                  {{ t('One source of truth for your life. Data you track in one area should inform the others: a transaction you log feeds your budget, budget progress shows up on your dashboard, and LifeHub AI can reason across all of it. Simple, private, and no-nonsense — the UI is deliberately brutal-simple so the data stays the hero.') }}
                </p>
              </div>
              <div class="rounded-card border-2 border-ink bg-surface p-5 shadow-soft">
                <h3 class="flex items-center gap-2 font-display text-base text-ink">
                ...
                <p class="mt-2 text-sm font-medium leading-relaxed text-ink-soft">
                  {{ t('LifeHub is a full-stack project built to prove the whole loop: a real authentication system, a real database, real-time tracking across domains, and a production-ready AI integration — end to end.') }}
                </p>
              </div>
            </div>
          </div>
        ...
        <section class="border-t-2 border-ink bg-surface">
          <div class="mx-auto max-w-6xl px-4 py-14 lg:px-6 lg:py-16">
            <h2 class="font-display text-2xl text-ink sm:text-3xl">{{ t('What’s inside') }}</h2>
            <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              @for (pillar of pillars; track pillar.title) {
                <div class="rounded-card border-2 border-ink bg-bg p-5 shadow-soft">
                  ...
                  <h3 class="mt-3 font-display text-base text-ink">{{ t(pillar.title) }}</h3>
                  <p class="mt-1.5 text-sm font-medium text-ink-soft">{{ t(pillar.text) }}</p>
                </div>
              }
            </div>
          </div>
        ...
        <section class="border-t-2 border-ink bg-bg">
          <div class="mx-auto max-w-6xl px-4 py-14 lg:px-6">
            <h2 class="text-center font-display text-2xl text-ink sm:text-3xl">{{ t('Built with') }}</h2>
        ...
        <section class="border-t-2 border-ink bg-primary">
          <div class="mx-auto max-w-3xl px-4 py-14 text-center lg:px-6">
            <h2 class="font-display text-3xl text-ink">{{ t('Curious what it feels like?') }}</h2>
...
export class AboutComponent {
  ...
  constructor() {
    effect(() => {
      this.i18n.lang();
      this.seo.setPage({
        title: this.i18n.t('About LifeHub — The Story Behind the Platform'),
